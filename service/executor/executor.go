...
func DefinitionToExecutor(logger log.Logger, p string, customCode int, d definition.Definition) (Executor, error) {
	var method string
	...
	if method == "" {
		return nil, DefinitionNoMethod.Error(d.Method, p)
	}
	if len(d.Consumes) <= 0 {
		return nil, DefinitionNoConsumes.Error(d.Method, p)
	}
	if len(d.Produces) <= 0 {
		return nil, DefinitionNoProduces.Error(d.Method, p)
	}
	if len(d.ErrorProduces) <= 0 {
		return nil, DefinitionNoErrorProduces.Error(d.Method, p)
	}
	if d.Function == nil {
		return nil, DefinitionNoFunction.Error(d.Method, p)
	}
	value := reflect.ValueOf(d.Function)
	if value.Kind() != reflect.Func {
		return nil, DefinitionInvalidFunctionType.Error(value.Type(), d.Method, p)
	}
	if customCode == 0 {
	...
	for _, ct := range d.Consumes {
		...
		if consumer := service.ConsumerFor(ct); consumer != nil {
			...
			return nil, DefinitionNoConsumer.Error(ct, d.Method, p)
		}
	}
	if consumeAll {
	...
	for _, ct := range d.Produces {
		...
		if producer := service.ProducerFor(ct); producer != nil {
			...
			return nil, DefinitionNoProducer.Error(ct, d.Method, p)
		}
	}
	if produceAll {
	...
	ps, err := generateParameters(logger, p, funcName, value.Type(), d.Parameters)
	if err != nil {
		return nil, err
	}
	c.parameters = ps
	rs, err := generateResults(logger, p, funcName, value.Type(), d.Results)
	if err != nil {
		return nil, err
	}
	c.results = rs
...
func generateParameters(logger log.Logger, path, funcName string, typ reflect.Type, ps []definition.Parameter) ([]parameter, error) {
	if typ.NumIn() != len(ps) {
	...
	for index, p := range ps {
		...
		if err := generator.Validate(param.name, param.defaultValue, param.targetType); err != nil {
			...
			logger.Errorf("Can't validate %s parameter of function %s: %s", order(index+1), funcName, err.Error())
			return nil, err
		}
		if len(param.operators) > 0 {
			if err := validateOperators(param.targetType, typ.In(index), param.operators); err != nil {
				logger.Errorf("Can't validate operators for %s parameter of function %s: %s", order(index+1), funcName, err.Error())
				return nil, err
			}
		}
...
func generateResults(logger log.Logger, path, funcName string, typ reflect.Type, rs []definition.Result) ([]result, error) {
	...
	for index, r := range rs {
		...
		if len(result.operators) > 0 {
			...
			if err := validateOperators(outType, LastOperatorOutType, result.operators); err != nil {
				logger.Errorf("Can't validate operators for %s result of function %s: %s", order(index+1), funcName, err.Error())
				return nil, err
			}
		...
		if err := handler.Validate(outType); err != nil {
			// Order from 0 is odd. So index+1.
			logger.Errorf("Can't validate %s result of function %s: %s", order(index+1), funcName, err.Error())
			return nil, err
		}
